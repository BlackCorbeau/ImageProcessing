...
# Выбираем семена для газона (зеленые области)
seeds = [(200, 150), (300, 200), (400, 180), (250, 250)]

# Применяем с цветным критерием
print("Применяем алгоритм с цветным критерием...")
regions_color = rg.grow_regions(image_rgb, seeds, rg.homogeneity_criterion_color, threshold=30)
...
plt.title('Выделение газона (цветной критерий)')
plt.axis('off')
...
plt.title('Выделение газона (критерий интенсивности)')
plt.axis('off')
...
area_intensity = np.sum(result_intensity[0])
area_variance = np.sum(result_variance[0])
area_gradient = np.sum(result_gradient[0])
